...
<html lang="en">
  <head>
    ...
    <title>{{$errorPageData->pageTitle ?? ''}}</title>
  </head>
  <style>
    img{
        height: 200px;
...
<div class="container center ">
  <h1> {{$errorPageData->pageTitle ?? ''}}</h1><hr>
</div>
</div>
<div class="container">

    @foreach(($errorPageData->linkTree ?? []) as $home)
        @if($home->children)
            <h1>{{$home->name}}</h1>
            <div class="row">
                @foreach($errorPageData->{lcfirst($home->name)}->values  as $errorComponet)
                    <div class="col-4">
                        <div class="card" style="width: 18rem;">
                            <div class="card-body">
                                {!! $errorComponet->content->value !!}
                            </div>
                        </div>
                    </div>
                @endforeach
    ...
    <hr>
    <div class="row">
        <div class="col-6" style="display: flex;"><h6>{{$errorPageData->pageTitle !=[] ? 'By :' : ''}}</h6><h6>{{$errorPageData->pageTitle}}</h6></div>
        <div class="col-6" style="display: flex;">
        <h6>{{$errorPageData->categories !=[] ? 'Categorized as :' : ''}}</h6>
        @foreach(($errorPageData->categories ?? []) as $categories)
